...
public class Prof_WorkEx_2 extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        userId = Integer.toString(AppPrefManager.getUserId(Prof_WorkEx_2.this));
        btn1 = (Button) findViewById(R.id.save_workexp);
        btn1.setOnClickListener(this);
        btn2 = (Button) findViewById(R.id.save_workk);
...
//                        //editText.setText(AppPrefManager.getUserIntroduction(Prof_Introduction.this));
                        ...
                        Toast.makeText(Prof_WorkEx_2.this, "successfully added", Toast.LENGTH_SHORT).show();
                        Intent i = new Intent(Prof_WorkEx_2.this,Profile_ok.class);
                        startActivity(i);
                        finish();
...
//        dbHandler.close();
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.sdate:
                new DatePickerDialog(Prof_WorkEx_2.this, date, myCalendar.get(Calendar.YEAR), myCalendar.get(Calendar.MONTH),
                        myCalendar.get(Calendar.DAY_OF_MONTH)).show();
                break;

            case R.id.edate:
                new DatePickerDialog(Prof_WorkEx_2.this, date1, myCalendar1.get(Calendar.YEAR), myCalendar1.get(Calendar.MONTH),
                        myCalendar1.get(Calendar.DAY_OF_MONTH)).show();
                break;

            case R.id.save_workexp:
            ...
            case R.id.save_workk:
                String company_lb=company.getText().toString();
                String title_lb=title.getText().toString();
                String location_lb=location.getText().toString();
                String jobdescription_lb=jobdescription.getText().toString();
                String var_startdat = updateLabel1();
                String var_enddat = "present";
                if (!company_lb.isEmpty() && !title_lb.isEmpty() && !location_lb.isEmpty()) {

                    workExp(userId,company_lb, title_lb, location_lb,var_startdat,var_enddat,jobdescription_lb);
                } else {
                    Toast.makeText(getApplicationContext(),"Please enter your details!", Toast.LENGTH_LONG).show();
                }
                break;
